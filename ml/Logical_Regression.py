...
def plot_logistic_regression(model, X_test, y_test, feature_name):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"media/logistic_regression_plot_{feature_name}.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"media/logistic_regression_confusion_matrix_{feature_name}.png")
    plt.close()
...
def training(file_path, target_column, train_ratio, random_state):
    ...
    plot_logistic_regression(model, X_test.numpy(), y_test.numpy(), feature_name=f"Feature {target_column}")

    # 保存模型
    model_path = f'media/logistic_regression_model_{target_column}.joblib'
