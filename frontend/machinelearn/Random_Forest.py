...
def plot_random_forest(model, X_test, y_test,random_state,feature_name):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"media/random_forest_plot_{random_state}.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"media/random_forest_confusion_matrix_{random_state}.png")
    plt.close()
...
def training3(file_path, target_column, train_ratio, random_state):
    # 使用示例
    ...
    plot_random_forest(model, X_test.numpy(), y_test.numpy(),random_state,feature_name=f"Feature {target_column}")

    # 保存模型
    model_path = f'media/random_forest_model_{random_state}.joblib'
