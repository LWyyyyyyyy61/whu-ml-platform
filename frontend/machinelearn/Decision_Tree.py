...
def plot_decision_tree(model, X_test, y_test, random_state,feature_name):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"media/decision_tree_plot_{random_state}.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"media/confusion_matrix_{random_state}.png")
    plt.close()
...
def training1(file_path,target_column,train_ratio,random_state):
    # 使用示例
    ...
    plot_decision_tree(model, X_test.numpy(), y_test.numpy(), random_state,feature_name=f"Feature {target_column}")
